	...
	git_buf *buf)
{
	...
	if (bld->write_cache.asize == 0 &&
	    (error = git_buf_grow(&bld->write_cache, entrycount * 72)) < 0)
		goto out;
	...
	for (i = 0; i < entries.length && !error; ++i) {
		...
		git_buf_printf(&bld->write_cache, "%o ", entry->attr);
		git_buf_put(&bld->write_cache, entry->filename, entry->filename_len + 1);
		git_buf_put(&bld->write_cache, (char *)entry->oid->id, GIT_OID_RAWSZ);

		if (git_buf_oom(&bld->write_cache)) {
			error = -1;
			goto out;
		}
	}
	...
	if ((error = git_repository_odb__weakptr(&odb, bld->repo)) == 0)
		error = git_odb_write(oid, odb, bld->write_cache.ptr, bld->write_cache.size, GIT_OBJECT_TREE);
